class Gastos {
  constructor({ 
    ...
    if (!id_viatico) throw new Error("El id_viatico es requerido para vincular el gasto a una cabecera");
    if (!id_categoria) throw new Error("La categoría del gasto es requerida");
    if (!monto || monto <= 0) throw new Error("El monto debe ser un valor positivo");
